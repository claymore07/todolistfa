...
<li class="list-group-item" id="todo-list-{{$todolist->id}}">
    ...
    <div class="buttons">
        <a href="{{route('todolists.show', $todolist->id)}}" class="btn btn-info show-task-modal btn-xs" data-action="{{route('todolists.tasks.store', $todolist->id)}}" data-title="{{$todolist->title}}" title="مدیریت لیست کارها">
